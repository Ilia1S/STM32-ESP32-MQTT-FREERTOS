...
uint8_t temperature = 0, humidity = 0, light = 0, thresholdsValue = 0, manualMode = 0, i = 0;
...
void callback(char* topic, byte* message, uint16_t length)
{
  ...
  if (String(topic) == "esp32/ledOutput")
  {
    ...
    {
      espToStm[0] = 1; //command type: 1 - LED state, 2 - thresholds, 3 - manual mode
      espToStm[1] = 0;
      Serial2.write(espToStm, 2);
    }
  }
  if (String(topic) == "esp32/thresholdsOutput")
  {
    ...
    espToStm[0] = 2; //1 - LED state, 2 - thresholds, 3 - manual mode
  ...
  if (String(topic) == "esp32/manualMode")
  {
    ...
    espToStm[0] = 3; //1 - LED state, 2 - thresholds, 3 - manual mode
